...
class MarketMakerUI:
    ...
    def __init__(self, symbol: str, center_price: float, levels: int, 
                 spread_pct: float, size: float, side_filter: str = None):
        """
    ...
    def display_startup_info(self, test_only: bool = False) -> None:
        ...
        if test_only:
            print(f"   Mode: TEST ONLY")
    
    def display_order_preview(self, orders: List[Tuple[str, float, float]], 
                            title: str = "Orders to place") -> None:
        """
    ...
    def display_order_adjustment_preview(self, new_center: float, orders: List[Tuple[str, float, float]]) -> None:
    ...
    def confirm_startup(self, orders: List[Tuple[str, float, float]], 
                       test_only: bool = False, yes: bool = False) -> bool:
        """
        ...
        if not test_only:
            ...
            print(f"   • Exit immediately (test mode)")
    ...
    def display_order_table(self, orders: List[Any], title: str = "Active Orders") -> None:
        ...
        for order in orders:
            ...
            print(f"{order_id:<12} {symbol:<10} {order_type:<15} {side:<4} {amount_abs:<15.6f} {price_str:<15}")
