...
class OrderManager:
    ...
    def __init__(self, symbol: str, levels: int, spread_pct: float, size: float,
                 side_filter: Optional[str] = None, client: Any = None):
        """
    ...
    def place_initial_orders(self, center_price: float, ui_callback=None) -> int:
        ...
        """
        if ui_callback:
            ui_callback(f"🚀 Placing initial orders around center price ${center_price:.6f}", "info")
        ...
        for side, amount, price in orders_to_place:
            try:
                ...
                if ui_callback:
                    ui_callback(f"✅ {side.upper()} POST-ONLY order placed: {amount} @ ${price:.6f}", "success")
                ...
                if "would have matched" in str(e).lower():
                    if ui_callback:
                        ui_callback(f"⚠️  {side.upper()} POST-ONLY order @ ${price:.6f} cancelled (would have matched existing order)", "warning")
                else:
                    if ui_callback:
    ...
    def cancel_all_orders(self, ui_callback=None) -> None:
        ...
        if order_ids and self.client:
            try:
                ...
                if ui_callback:
                    ui_callback(f"✅ Successfully submitted bulk cancellation for {len(order_ids)} orders", "success")
    ...
    def check_and_replenish_orders(self, ui_callback=None) -> int:
        ...
        try:
            ...
            if missing_orders:
                if ui_callback:
                    ui_callback(f"🔄 Found {len(missing_orders)} cancelled orders - replenishing...", "info")
                ...
                for order_id, order_info in missing_orders:
                    ...
                    if ui_callback:
                        ui_callback(f"   Replenishing {side.upper()} order: {amount} @ ${price:.6f}", "info")
                    ...
                    try:
                        ...
                        if ui_callback:
                            ui_callback(f"   ✅ Replenished: {side.upper()} POST-ONLY {amount} @ ${price:.6f} (ID: {new_order_id})", "success")
                        ...
                        if "would have matched" in str(e).lower():
                            if ui_callback:
                                ui_callback(f"   ⚠️  {side.upper()} POST-ONLY replenishment @ ${price:.6f} cancelled (would have matched existing order)", "warning")
                        else:
                            if ui_callback:
                                ui_callback(f"   ❌ Failed to replenish {side.upper()} order: {e}", "error")
                ...
                if self._check_count % 10 == 0:  # Every 10th check (5 minutes)
                    ...
                    if ui_callback:
                        ui_callback(f"✅ Order check #{self._check_count}: All {tracked_count} orders still active", "info")
