...
class OrderUpdateValidator:
    ...
    def validate_update_request(self, order_id: int, price: Optional[float] = None,
                              amount: Optional[float] = None, delta: Optional[float] = None) -> OrderUpdateRequest:
        """
    ...
    def validate_new_amount_calculation(self, request: OrderUpdateRequest, current_order: any) -> Decimal:
        """
    ...
    def validate_new_price(self, request: OrderUpdateRequest, current_order: any) -> Optional[float]:
        """
        ...
        if request.has_price_update():
            ...
            logger.debug(f"Keeping existing price: ${current_price:.6f}" if current_price else "No price (market order)")
