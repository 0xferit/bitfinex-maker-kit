    ...
    async def _cancel_background_tasks(self) -> None:
        ...
        if self.background_tasks:
            try:
                ...
                logger.warning(f"Some background tasks did not complete within {self.config.task_cleanup_timeout}s")
...
@asynccontextmanager
async def managed_event_loop(config: Optional[EventLoopConfig] = None):
    """
...
class AutoMarketMakerEventLoop:
    ...
    def __init__(self, connection_manager: AsyncWebSocketConnectionManager,
        ...
        self.event_loop_manager = AsyncEventLoopManager(
            event_loop_config or EventLoopConfig()
        )
        
        # Market making state
        self.market_making_active = False
        self.replenish_task: Optional[asyncio.Task] = None
    ...
    async def start_market_making(self, replenish_interval: float = 30.0) -> None:
        ...
        self.replenish_task = self.event_loop_manager.create_task(
            self._replenishment_loop(replenish_interval),
            name="order_replenishment"
        )
