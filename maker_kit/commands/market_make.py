...
from ..utilities.market_data import validate_center_price, resolve_center_price, suggest_price_centers
from ..utilities.orders import submit_order


def market_make_command(symbol: str = DEFAULT_SYMBOL, center: Optional[str] = None, 
                       levels: int = DEFAULT_LEVELS, spread: float = DEFAULT_SPREAD_PCT, 
                       size: float = DEFAULT_ORDER_SIZE, dry_run: bool = False, 
                       buy_only: bool = False, sell_only: bool = False,
    ...
    if center:
        ...
        market_make(symbol, center_price, levels, spread, size, dry_run, side_filter, ignore_validation, yes)
    else:
        ...
        if centers:
            ...
            print(f"maker-kit market-make --symbol {symbol} --center PRICE --levels {levels} --spread {spread} --size {size}{side_suffix}")
            print(f"\nExample using mid price:")
            print(f"maker-kit market-make --symbol {symbol} --center {centers['mid_price']:.6f} --levels {levels} --spread {spread} --size {size}{side_suffix}")
            print(f"\nExample using mid-range:")
            print(f"maker-kit market-make --symbol {symbol} --center mid-range --levels {levels} --spread {spread} --size {size}{side_suffix}")
