...
class ConfirmationHandler:
    ...
    def __init__(self, prompt_function: Optional[Callable[[str], bool]] = None):
        """
...
class CommandExecutor:
    ...
    def execute_command(self, command: Command, options: ExecutionOptions) -> CommandResult:
        ...
        try:
            ...
            result = CommandResult.failure(f"Executor error: {str(e)}")
    ...
    def execute_batch(self, commands: List[Command], options: ExecutionOptions) -> List[CommandResult]:
        """
            ...
            try:
                ...
                if result.is_success():
                    ...
                    if options.fail_fast and result.status == CommandStatus.FAILED:
                        logger.warning(f"Stopping batch execution due to failure: {result.error_message}")
                        break
                        
            except Exception as e:
                logger.error(f"Error executing command {i+1} in batch: {e}")
                error_result = CommandResult.failure(f"Batch execution error: {str(e)}")
    ...
    def undo_command(self, command: Command, options: ExecutionOptions) -> CommandResult:
        ...
        try:
            ...
            return CommandResult.failure(f"Undo error: {str(e)}")
    ...
    def _execute_with_timeout(self, command: Command, context: CommandContext, 
