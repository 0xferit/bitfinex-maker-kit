...
from .base_command import TransactionalCommand, BatchCommand, CommandContext
...
class CancelOrderCommand(TransactionalCommand):
    ...
    def __init__(self, order_id: Union[int, str]):
        """
        ...
        self.order_id_obj: Optional[OrderId] = None
        self.cancelled_order_data: Optional[dict] = None
    ...
    def validate(self, context: CommandContext) -> ValidationResult:
        ...
        try:
            ...
            result.add_error(f"Invalid order ID: {str(e)}")
            return result
        ...
        try:
            ...
            if not order_exists:
                result.add_warning(f"Order {self.order_id_obj} not found in active orders (may have been filled or already cancelled)")
        
        except Exception as e:
            result.add_warning(f"Could not verify order existence: {str(e)}")
    ...
    def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            return CommandResult.failure(f"Cancellation error: {str(e)}")
...
class CancelOrdersByCriteriaCommand(BatchCommand):
    ...
    def __init__(self, symbol: Optional[str] = None, side: Optional[str] = None,
                 price_below: Optional[float] = None, price_above: Optional[float] = None,
                 size: Optional[float] = None, fail_fast: bool = False):
        """
    ...
    def validate(self, context: CommandContext) -> ValidationResult:
        ...
        if (self.price_below is not None and self.price_above is not None 
        ...
        try:
            ...
            result.add_error(f"Error finding matching orders: {str(e)}")
    ...
    def _execute_single_operation(self, order: any, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            return CommandResult.failure(f"Error cancelling order: {str(e)}")
