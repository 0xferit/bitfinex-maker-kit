...
class MarketMakeCommand(BatchCommand):
    ...
    def __init__(self, symbol: str, center_price: float, levels: int, 
                 spread_pct: float, size: float, side_filter: Optional[str] = None):
        """
        ...
        self.symbol_obj: Optional[Symbol] = None
        self.center_price_obj: Optional[Price] = None
        self.size_obj: Optional[Amount] = None
    ...
    def validate(self, context: CommandContext) -> ValidationResult:
        ...
        try:
            ...
            result.add_error(f"Invalid domain object: {str(e)}")
            return result
        ...
        try:
            self.order_generator = OrderGenerator(self.levels, self.spread_pct, self.size_float, self.side_filter)
            ...
            result.add_error(f"Error generating orders: {str(e)}")
            return result
        ...
        for i, (side, amount, price) in enumerate(self.generated_orders):
            try:
                ...
                if not cmd_validation.is_valid:
                    result.add_error(f"Order {i+1} validation failed: {cmd_validation.get_error_summary()}")
                ...
                result.add_error(f"Error validating order {i+1}: {str(e)}")
            ...
            if ticker:
                ...
                if bid > 0 and ask > 0:
                    ...
                    if center_deviation > 5:  # 5% deviation
                        result.add_warning(f"Center price deviates {center_deviation:.2f}% from mid price ${mid_price:.6f}")
                    ...
                    if min_price < bid * 0.5:
                        result.add_warning(f"Some buy orders very far below market (min: ${min_price:.6f})")
                    
                    if max_price > ask * 2:
                        result.add_warning(f"Some sell orders very far above market (max: ${max_price:.6f})")
        
        except Exception as e:
            result.add_warning(f"Could not validate market conditions: {str(e)}")
        
        # Capital requirement estimation
        try:
            capital_info = self.order_generator.calculate_total_capital_required(self.center_price_float)
            
            result.add_warning(f"Estimated capital required: "
            ...
            result.add_warning(f"Could not calculate capital requirements: {str(e)}")
    ...
    def _execute_single_operation(self, place_cmd: PlaceOrderCommand, context: CommandContext) -> CommandResult:
        """
    ...
    def undo(self, context: CommandContext) -> CommandResult:
        ...
        orders_to_cancel = []
        for i, result in enumerate(self.partial_results):
        ...
        for order_id in orders_to_cancel:
            try:
                ...
                failed_cancellations.append(f"{order_id}: {str(e)}")
    ...
    def get_preview(self, context: CommandContext) -> str:
        ...
        return (f"Place {len(self.generated_orders)} market making orders: "
