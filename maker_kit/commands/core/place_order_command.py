...
from .base_command import TransactionalCommand, CommandContext
...
class PlaceOrderCommand(TransactionalCommand):
    ...
    def __init__(self, symbol: str, side: str, amount: float, 
        ...
        self.amount_obj: Optional[Amount] = None
        self.price_obj: Optional[Price] = None
        
        # Will be set during execution
        self.placed_order_id: Optional[OrderId] = None
        self.placed_order_data: Optional[dict] = None
            ...
            if ticker:
                ...
                if bid > 0 and ask > 0:
                    ...
                    if self.side == 'buy' and price_val >= ask:
                        result.add_warning(f"Buy price ${price_val:.6f} at or above ask ${ask:.6f} may execute immediately")
                    elif self.side == 'sell' and price_val <= bid:
                        result.add_warning(f"Sell price ${price_val:.6f} at or below bid ${bid:.6f} may execute immediately")
                    
                    # Check for reasonable price ranges
                    spread_pct = ((ask - bid) / bid) * 100 if bid > 0 else 0
                    if spread_pct > 5.0:  # 5% spread
                        result.add_warning(f"Wide spread detected: {spread_pct:.2f}% - verify price is correct")
    ...
    def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            return CommandResult.failure(f"Order placement error: {str(e)}")
    ...
    def undo(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            if success:
                ...
                return CommandResult.failure(f"Failed to cancel order {self.placed_order_id}: {result}")
