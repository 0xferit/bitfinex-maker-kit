...
from .command_result import CommandResult, ValidationResult, CommandStatus
    ...
    def execute_with_validation(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            if not validation_result.is_valid:
                self.logger.warning(f"Command validation failed: {validation_result.get_error_summary()}")
            ...
            if context.dry_run:
                ...
                result = CommandResult.success(data={"dry_run": True, "preview": self.get_preview(context)})
            else:
                # Execute command
            ...
            if result.is_success():
                self.logger.info(f"Command executed successfully: {self.name} ({self.execution_time:.3f}s)")
            else:
            ...
            result = CommandResult.failure(f"Unexpected error: {str(e)}")
    ...
    def execute_batch_operation(self, operations: list, context: CommandContext) -> CommandResult:
        ...
        for i, operation in enumerate(operations):
            try:
                ...
                error_result = CommandResult.failure(f"Operation {i+1} failed: {str(e)}")
        ...
        if failed_operations == 0:
            ...
            return CommandResult.failure(
                f"{failed_operations} of {total_operations} operations failed",
                data=summary
            )
