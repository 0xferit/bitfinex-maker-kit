...
def update_single_order(order_id: int, price: Optional[float] = None, 
                       amount: Optional[float] = None, delta: Optional[float] = None,
                       dry_run: bool = False, yes: bool = False, use_cancel_recreate: bool = False):
    ...
    if use_cancel_recreate:
        print(f"   Method: Cancel-and-recreate (riskier - may lose order if recreation fails)")
    else:
        print(f"   Method: WebSocket atomic update (safer - preserves POST_ONLY)")
...
def update_orders_by_criteria(filter_size: Optional[float] = None, filter_direction: Optional[str] = None,
                             filter_symbol: Optional[str] = None, price: Optional[float] = None,
                             amount: Optional[float] = None, delta: Optional[float] = None,
                             dry_run: bool = False, yes: bool = False, use_cancel_recreate: bool = False):
    ...
    if use_cancel_recreate:
        print(f"Method: Cancel-and-recreate (riskier - may lose orders if recreation fails)")
    else:
        print(f"Method: WebSocket atomic update (safer - preserves POST_ONLY)")
    ...
    print()
    if not yes and not confirm_action(f"Do you want to update these {len(matching_orders)} orders ({changes_desc})?"):
...
def update_command(order_id: Optional[int] = None, price: Optional[float] = None, 
                   amount: Optional[float] = None, delta: Optional[float] = None,
                   filter_size: Optional[float] = None, filter_direction: Optional[str] = None,
                   filter_symbol: Optional[str] = None, dry_run: bool = False, yes: bool = False,
    ...
    if not order_id and not has_criteria:
        print_error("Must specify either order_id or filter criteria (--filter-size, --filter-direction, --filter-symbol)")
        return
    ...
    if order_id:
        ...
        update_orders_by_criteria(filter_size, filter_direction, filter_symbol, 
                                 price, amount, delta, dry_run, yes, use_cancel_recreate) 