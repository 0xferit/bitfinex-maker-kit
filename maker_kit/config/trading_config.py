...
from ..utilities.constants import (
    ...
    POST_ONLY_FLAG, MIN_ORDER_SPACING_PCT, REPLENISH_INTERVAL_SECONDS
)
...
class TradingConfig:
    ...
    def _validate_configuration(self) -> None:
        ...
        if not self.min_levels <= self.default_levels <= self.max_levels:
            raise ValueError(f"Default levels {self.default_levels} must be between {self.min_levels} and {self.max_levels}")
        
        # Validate spread
        if not self.min_spread_pct <= self.default_spread_pct <= self.max_spread_pct:
            raise ValueError(f"Default spread {self.default_spread_pct}% must be between {self.min_spread_pct}% and {self.max_spread_pct}%")
        
        # Validate order size
        if not self.min_order_size <= self.default_order_size <= self.max_order_size:
            raise ValueError(f"Default order size {self.default_order_size} must be between {self.min_order_size} and {self.max_order_size}")
    ...
    def _apply_environment_overrides(self) -> None:
        ...
        # Default symbol
        if symbol := os.getenv("MAKER_KIT_DEFAULT_SYMBOL"):
