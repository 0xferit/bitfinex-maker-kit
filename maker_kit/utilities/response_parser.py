...
class OrderResponseParser:
    ...
    @staticmethod
    def extract_order_id(response: Any) -> Optional[Union[int, str]]:
        """
        ...
        try:
            ...
            if hasattr(response, 'data'):
                ...
                if hasattr(response.data, '__dict__'):
                    logger.debug(f"{context_prefix}  Data attributes: {list(response.data.__dict__.keys())}")
                    
            if isinstance(response, list):
                logger.debug(f"{context_prefix}  List length: {len(response)}")
                if response and hasattr(response[0], '__dict__'):
                    logger.debug(f"{context_prefix}  First item attributes: {list(response[0].__dict__.keys())}")
...
class OrderTracker:
    ...
    def __init__(self):
        self.tracked_orders: Dict[Union[int, str], Dict[str, Any]] = {}
    
    def track_order_from_response(self, response: Any, side: str, amount: float, 
                                 price: float, symbol: str = "") -> Union[int, str]:
        """
