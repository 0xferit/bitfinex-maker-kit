...
class CancelRecreateStrategy(OrderUpdateStrategy):
    ...
    def execute_update(self, request: OrderUpdateRequest, 
                      current_order: Any, client: Any) -> OrderUpdateResult:
        """
        ...
        logger.debug(f"Original: {side.upper()} {abs(float(current_order.amount))} @ ${current_price}")
    ...
    def _validate_recreation_parameters(self, symbol: str, side: str, 
            ...
            try:
                ...
                if "not found" in error_msg:
                    ...
                    if attempt == max_retries - 1:
                        raise OrderSubmissionError(f"Failed to cancel order after {max_retries} attempts: {e}")
                    else:
                        logger.warning(f"Cancel attempt {attempt + 1} failed: {e}")
        
        return False
    
    def _create_new_order(self, symbol: str, side: str, amount: float, 
                         price: float, client: Any) -> Any:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.debug(f"Creating new order: {side.upper()} {amount} @ ${price:.6f} (attempt {attempt + 1})")
                ...
                logger.info(f"Successfully created new order: {side.upper()} {amount} @ ${price:.6f}")
                return new_order
