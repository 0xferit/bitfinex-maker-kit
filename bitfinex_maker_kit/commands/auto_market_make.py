...
from ..utilities.market_data import resolve_center_price, validate_center_price
...
class AutoMarketMaker:
    ...
    def __init__(
        ...
        self.order_manager = self.container.create_order_manager(
            symbol, levels, spread_pct, size, side_filter
        )
        self.ui = self.container.create_market_maker_ui(
            symbol, center_price, levels, spread_pct, size, side_filter
        )
        self.websocket_handler = self.container.create_websocket_handler(self.order_manager)
        self.order_generator = self.container.create_order_generator(
            levels, spread_pct, size, side_filter
        )
    ...
    def validate_initial_price(self):
        ...
        is_valid, range_info = validate_center_price(
            self.symbol, self.initial_center, self.ignore_validation
        )
        if not is_valid:
            if range_info:
                error_msg = (
                    f"Invalid center price: ${self.initial_center:.6f} is outside the current bid-ask spread. "
