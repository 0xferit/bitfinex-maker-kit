...
from maker_kit.exchanges.bitfinex_client import BitfinexClientWrapper, create_wrapper_client
from maker_kit.auth import create_client
...
class TestBitfinexClientWrapper(unittest.TestCase):
    ...
    @patch('maker_kit.exchanges.bitfinex_client.Client')
    ...
    def test_wrapper_hardcoded_post_only_flag(self, mock_client_class):
        ...
        for side, amount, price in test_cases:
            with self.subTest(side=side, amount=amount, price=price):
                ...
                self.assertEqual(call_kwargs['flags'], 4096,
    ...
    def test_wrapper_no_bypass_methods(self):
        ...
        expected_safe_methods = {
            'submit_order', 'get_orders', 'cancel_order', 'cancel_order_multi',
            'get_wallets', 'get_ticker', 'get_trades', 'wss'
        }
        order_related_methods = {method for method in public_methods 
        ...
        self.assertEqual(len(unexpected_methods), 0, 
                        f"Wrapper exposes unexpected methods: {unexpected_methods}")
...
class TestApplicationLayerIntegration(unittest.TestCase):
    ...
    def test_application_layer_isolation(self):
        ...
        from maker_kit import orders, market_making, auto_market_maker, wallet, market_data
        ...
        for module in modules_to_check:
            ...
            self.assertNotIn('from bfxapi import Client', source,
                           f"Module {module.__name__} should not import raw Bitfinex Client")
            self.assertNotIn('import bfxapi', source,
            ...
            if 'client' in source.lower():
                self.assertIn('create_client', source,
...
class TestWrapperFactory(unittest.TestCase):
    ...
    @patch('maker_kit.exchanges.bitfinex_client.BitfinexClientWrapper')
