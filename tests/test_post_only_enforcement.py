...
from maker_kit.orders import submit_order
...
class TestCentralizedPostOnlyEnforcement(unittest.TestCase):
    ...
    @patch('maker_kit.orders.create_client')
    def test_submit_order_limit_enforces_post_only(self, mock_create_client):
        """
        ...
        """
        mock_create_client.return_value = self.mock_wrapper_client
        ...
        self.mock_wrapper_client.submit_order.assert_called_once_with(
            "tPNKUSD", "buy", 10.0, 0.100
        )
    
    @patch('maker_kit.orders.create_client')
    def test_submit_order_market_no_post_only(self, mock_create_client):
    ...
    @patch('maker_kit.orders.create_client')
    def test_submit_order_hardcoded_post_only_flag(self, mock_create_client):
        """
    ...
    def test_submit_order_input_validation(self):
        ...
        from maker_kit.constants import ValidationError
    ...
    @patch('maker_kit.orders.create_client')
    def test_submit_order_post_only_rejection_handling(self, mock_create_client):
        ...
        self.mock_wrapper_client.submit_order.side_effect = Exception("Order would have matched existing order")
...
class TestArchitecturalEnforcement(unittest.TestCase):
    ...
    def test_all_order_functions_use_submit_order(self):
        ...
        from maker_kit import orders, market_making, auto_market_maker
        
        # These modules should import submit_order
        self.assertTrue(hasattr(orders, 'submit_order'), 
                       "orders module should have submit_order function")
        
        # Check that market_making imports submit_order
        import inspect
        market_making_source = inspect.getsource(market_making)
        self.assertIn('submit_order', market_making_source,
                     "market_making should import submit_order")
        
        # Check that auto_market_maker imports submit_order  
        auto_mm_source = inspect.getsource(auto_market_maker)
        self.assertIn('submit_order', auto_mm_source,
                     "auto_market_maker should import submit_order")
    ...
    def test_no_duplicate_order_submission_logic(self):
        ...
        """
        import inspect
        from maker_kit import market_making, auto_market_maker
        
        # These modules should NOT have direct client.rest.auth.submit_order calls
        market_making_source = inspect.getsource(market_making)
        self.assertNotIn('client.rest.auth.submit_order', market_making_source,
                        "market_making should not have direct order submission")
        
        auto_mm_source = inspect.getsource(auto_market_maker)
        self.assertNotIn('client.rest.auth.submit_order', auto_mm_source,
    ...
    def test_submit_order_function_documentation(self):
        ...
        self.assertIn('enforces post_only', docstring,
                     "submit_order should document POST_ONLY enforcement")
        self.assertIn('centralized', docstring,
                     "submit_order should document its centralized role")
        self.assertIn('wrapper', docstring,
                     "submit_order should document that it uses the wrapper")
