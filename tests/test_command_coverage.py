...
class TestCommandCoverage:
    ...
    def test_all_commands_have_unit_tests(self, all_commands):
        ...
        for command in all_commands:
            ...
            if command_name not in test_content:
    ...
    def test_all_commands_have_integration_tests(self, all_commands):
        ...
        for command in all_commands:
            ...
            if f"test_{command_name.replace('_command', '')}" not in test_content:
    ...
    def test_all_commands_have_paper_trading_tests(self, all_commands):
        ...
        assert not missing_paper_tests, f"Commands missing paper trading tests: {missing_paper_tests}"
        ...
        for command in all_commands:
            ...
            for param in parameters:
                if param in ["dry_run", "symbol"]:  # These should always be tested
                    assert param in test_content, f"Parameter '{param}' not tested for {command_name}"
...
class TestCoverageMetrics:
    ...
    def test_all_source_files_are_covered(self):
        ...
        untested_files = [f for f in untested_files if not any(exc in f for exc in allowed_untested)]
    ...
    def test_command_imports_are_complete(self):
        ...
        with open(commands_init, 'r') as f:
            content = f.read()
        ...
        assert not missing_command_tests, f"Imported commands missing from tests: {missing_command_tests}"
