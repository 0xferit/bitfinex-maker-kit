...
class BenchmarkRunner:
    ...
    async def run_benchmark(self, test_name: str, operation_func, 
                          iterations: int = 1000, 
    ...
    def export_results(self, format: str = 'json') -> str:
        ...
        if format == 'json':
            ...
            for r in self.results:
                lines.append(f"{r.test_name},{r.operations_per_second:.2f},"
...
class TestDomainObjectBenchmarks:
    ...
    async def test_symbol_creation_benchmark(self, benchmark_runner):
        ...
        assert result.operations_per_second >= 5000, f"Symbol creation too slow: {result.operations_per_second:.1f} ops/sec"
        assert result.avg_time_per_op < 0.001, f"Average symbol creation time too high: {result.avg_time_per_op*1000:.2f}ms"
        assert result.success_rate >= 0.99, f"Symbol creation success rate too low: {result.success_rate:.3f}"
    ...
    async def test_price_arithmetic_benchmark(self, benchmark_runner):
        ...
        # Assert performance requirements
        assert result.operations_per_second >= 2000, f"Price arithmetic too slow: {result.operations_per_second:.1f} ops/sec"
        assert result.avg_time_per_op < 0.002, f"Average price arithmetic time too high: {result.avg_time_per_op*1000:.2f}ms"
    ...
    async def test_amount_operations_benchmark(self, benchmark_runner):
        ...
        # Assert performance requirements
        assert result.operations_per_second >= 3000, f"Amount operations too slow: {result.operations_per_second:.1f} ops/sec"
    ...
    async def test_order_id_operations_benchmark(self, benchmark_runner):
        ...
        # Assert performance requirements
        assert result.operations_per_second >= 4000, f"Order ID operations too slow: {result.operations_per_second:.1f} ops/sec"
...
class TestTradingServiceBenchmarks:
    ...
    async def test_order_placement_benchmark(self, benchmark_runner, trading_service):
        ...
        # Assert performance requirements
        assert result.operations_per_second >= 100, f"Order placement too slow: {result.operations_per_second:.1f} ops/sec"
        assert result.avg_time_per_op < 0.1, f"Average order placement time too high: {result.avg_time_per_op*1000:.2f}ms"
        assert result.success_rate >= 0.95, f"Order placement success rate too low: {result.success_rate:.3f}"
    ...
    async def test_order_cancellation_benchmark(self, benchmark_runner, trading_service):
        ...
        # Assert performance requirements
        assert result.operations_per_second >= 80, f"Order cancellation too slow: {result.operations_per_second:.1f} ops/sec"
        assert result.success_rate >= 0.90, f"Order cancellation success rate too low: {result.success_rate:.3f}"
...
class TestCacheBenchmarks:
    ...
    async def test_cache_set_benchmark(self, benchmark_runner, cache_service):
        ...
        assert result.operations_per_second >= 1000, f"Cache set too slow: {result.operations_per_second:.1f} ops/sec"
        assert result.avg_time_per_op < 0.005, f"Average cache set time too high: {result.avg_time_per_op*1000:.2f}ms"
        assert result.success_rate >= 0.99, f"Cache set success rate too low: {result.success_rate:.3f}"
    ...
    async def test_cache_get_benchmark(self, benchmark_runner, cache_service):
        ...
        assert result.operations_per_second >= 5000, f"Cache get too slow: {result.operations_per_second:.1f} ops/sec"
        assert result.avg_time_per_op < 0.001, f"Average cache get time too high: {result.avg_time_per_op*1000:.2f}ms"
        assert result.success_rate >= 0.99, f"Cache get success rate too low: {result.success_rate:.3f}"
    ...
    async def test_cache_get_or_set_benchmark(self, benchmark_runner, cache_service):
        ...
        assert result.operations_per_second >= 500, f"Cache get_or_set too slow: {result.operations_per_second:.1f} ops/sec"
        assert result.success_rate >= 0.95, f"Cache get_or_set success rate too low: {result.success_rate:.3f}"
    ...
    async def test_cache_mixed_operations_benchmark(self, benchmark_runner, cache_service):
        ...
        assert result.operations_per_second >= 800, f"Mixed cache operations too slow: {result.operations_per_second:.1f} ops/sec"
        assert result.success_rate >= 0.90, f"Mixed cache operations success rate too low: {result.success_rate:.3f}"
...
class TestMemoryBenchmarks:
    ...
    async def test_domain_object_memory_benchmark(self, benchmark_runner):
        ...
        assert memory_per_object < 1000, f"Domain objects use too much memory: {memory_per_object:.1f} bytes per object"
        assert result.operations_per_second >= 1000, f"Domain object creation too slow: {result.operations_per_second:.1f} ops/sec"
    ...
    async def test_cache_performance_regression(self, benchmark_runner):
        ...
        try:
            ...
            async def cache_operation():
                ...
                if counter % 5 == 0:
                    await cache_service.set('regression', f'key_{counter % 200}', f'new_value_{counter}')
                else:
...
class TestComprehensiveBenchmarkSuite:
    ...
    async def test_full_system_benchmark(self, benchmark_runner):
        ...
        try:
            ...
            assert result.operations_per_second >= 20, f"Full system operations too slow: {result.operations_per_second:.1f} ops/sec"
            assert result.success_rate >= 0.90, f"Full system success rate too low: {result.success_rate:.3f}"
            assert result.avg_time_per_op < 0.5, f"Average system operation time too high: {result.avg_time_per_op*1000:.2f}ms"
            
            # Check memory efficiency for complex operations
            assert result.memory_used_mb < 50, f"Full system operations use too much memory: {result.memory_used_mb:.1f}MB"
    ...
    async def test_stress_benchmark_suite(self, benchmark_runner):
        ...
        for load_config in load_levels:
            ...
            min_expected_ops = load_config['concurrency'] * 0.7  # Allow 30% degradation under stress
