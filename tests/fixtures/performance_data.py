...
class PerformanceFixtures:
    ...
    @classmethod
    def create_profile_data(cls, function_name: str, 
                          scenario: str = 'normal') -> ProfileDataFixture:
    ...
    def create_regression_test_data(cls) -> Dict[str, List[MetricsFixture]]:
        ...
        for i in range(10):
            hit_ratio_degradation = baseline.cache_hit_ratio * (1 - (i * 0.05))  # 5% degradation per step
            metrics = MetricsFixture(
                timestamp=baseline.timestamp + (i * 3600),
                api_calls_total=baseline.api_calls_total,
                api_response_time_avg=baseline.api_response_time_avg,
    ...
    @classmethod
    def create_time_series_data(cls, duration_hours: int = 24, 
                              interval_minutes: int = 5) -> List[MetricsFixture]:
