...
from ..fixtures.trading_data import TradingFixtures, OrderFixture
from ..fixtures.performance_data import PerformanceFixtures, MetricsFixture
...
class MockTradingService:
    ...
    def __init__(self, initial_balance: Optional[Dict[str, float]] = None,
                 order_execution_delay: float = 0.0):
        """
    ...
    async def place_order(self, symbol, amount, price, side, 
                         order_type: str = "EXCHANGE LIMIT", **kwargs) -> Dict[str, Any]:
        """Mock place order operation."""
        self._record_operation('place_order', symbol=str(symbol), 
                             amount=str(amount), price=str(price), side=side)
    ...
    async def cancel_order(self, order_id: str, symbol=None) -> Dict[str, Any]:
        ...
        self._record_operation('cancel_order', order_id=order_id, symbol=str(symbol) if symbol else None)
        await self._simulate_delay()
    ...
    async def update_order(self, order_id: str, new_amount=None, 
                          new_price=None, symbol=None) -> Dict[str, Any]:
        """Mock update order operation."""
        self._record_operation('update_order', order_id=order_id, 
    ...
    def simulate_order_execution(self, order_id: int, execution_price: Optional[str] = None):
...
class MockCacheService:
    ...
    async def get_or_set(self, namespace: str, key: str, 
                        fetch_func: Callable, ttl: Optional[float] = None) -> Any:
...
class MockPerformanceMonitor:
    ...
    def track_api_call(self, method: str, endpoint: str, 
    ...
    def get_current_metrics(self) -> MetricsFixture:
        ...
        self.current_metrics.cache_misses_total = self.recorded_counters.get('cache.misses_total', 0)
        
        # Calculate cache hit ratio
        total_cache_ops = self.current_metrics.cache_hits_total + self.current_metrics.cache_misses_total
        if total_cache_ops > 0:
            self.current_metrics.cache_hit_ratio = self.current_metrics.cache_hits_total / total_cache_ops
...
class MockTimingContext:
    ...
    def __init__(self, monitor: MockPerformanceMonitor, 
                 operation_name: str, labels=None):
...
def create_mock_trading_service(scenario: str = 'normal') -> MockTradingService:
    ...
    scenarios = {
        'normal': {
            'initial_balance': {'USD': 10000.0, 'BTC': 1.0},
