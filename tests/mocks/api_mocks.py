...
class MockBitfinexAPI:
    ...
    def __init__(
        self, response_delay: float = 0.0, error_rate: float = 0.0, rate_limit_enabled: bool = False
    ):
        """
        Initialize mock API.
    ...
    async def get_orderbook(self, symbol: str = "tBTCUSD", precision: str = "P0") -> dict[str, Any]:
    ...
    async def get_trades(self, symbol: str = "tBTCUSD", limit: int = 50) -> list[dict[str, Any]]:
    ...
    async def submit_order(
        ...
        **kwargs,
        ...
        response = self.responses.create_order_response(
            order_id=order_id, symbol=symbol, amount=amount, price=price, side=side, type=order_type
        )
        return response.data

    async def cancel_order(self, order_id: int, symbol: str | None = None) -> dict[str, Any]:
        ...
        response = self.responses.create_order_response(order_id=order_id, status="CANCELED")
        return response.data

    async def get_orders(self, symbol: str | None = None) -> list[dict[str, Any]]:
...
class MockWebSocketConnection:
    ...
    def __init__(self, auto_connect: bool = True, message_delay: float = 0.0):
        """
    ...
    async def connect(self, uri: str):
        ...
        self.events.append({"type": "connect", "uri": uri, "timestamp": time.time()})
    ...
    async def send(self, message: str | dict[str, Any]):
        ...
        self.sent_messages.append({"message": message, "timestamp": time.time()})

        self.events.append({"type": "send", "message": message, "timestamp": time.time()})
    ...
    async def recv(self) -> str:
        ...
        self.events.append({"type": "recv", "message": message, "timestamp": time.time()})
    ...
    async def close(self):
        ...
        self.events.append({"type": "close", "timestamp": time.time()})

        if self.on_close:
            await self.on_close()

    async def simulate_message(self, message: str | dict[str, Any]):
    ...
    async def simulate_error(self, error_message: str):
        ...
        self.events.append({"type": "error", "error": error_message, "timestamp": time.time()})
...
class MockAPIResponse:
    ...
    def __init__(
        self, status_code: int = 200, data: Any = None, headers: dict[str, str] | None = None
    ):
...
def create_mock_api(scenario: str = "normal") -> MockBitfinexAPI:
    ...
    """
    scenarios = {
        "normal": {"response_delay": 0.0, "error_rate": 0.0, "rate_limit_enabled": False},
        "slow": {"response_delay": 0.5, "error_rate": 0.0, "rate_limit_enabled": False},
        "errors": {
            "response_delay": 0.0,
            "error_rate": 0.2,  # 20% error rate
            "rate_limit_enabled": False,
        },
        "rate_limited": {"response_delay": 0.0, "error_rate": 0.0, "rate_limit_enabled": True},
        "stress": {"response_delay": 1.0, "error_rate": 0.1, "rate_limit_enabled": True},
    }
...
def create_mock_websocket(scenario: str = "normal") -> MockWebSocketConnection:
    ...
    """
    scenarios = {
        "normal": {"auto_connect": True, "message_delay": 0.0},
        "slow": {"auto_connect": True, "message_delay": 0.1},
        "disconnected": {"auto_connect": False, "message_delay": 0.0},
    }
