...
from maker_kit.services.container import ServiceContainer, create_service_container
from maker_kit.services.cache_service import CacheService, create_cache_service
from maker_kit.services.performance_monitor import PerformanceMonitor, create_performance_monitor
from maker_kit.utilities.profiler import PerformanceProfiler, create_profiler
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "load: mark test as load test (stress testing)"
    )
    config.addinivalue_line(
...
@pytest.fixture
async def service_container(mock_bitfinex_client, test_config) -> AsyncGenerator[ServiceContainer, None]:
...
def websocket_messages() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            'event': 'update',
            'channel': 'ticker',
            'data': [1, 50000.0, 0.1, 50050.0, 0.2, 100.0, 0.002, 50000.0, 1000.0, 51000.0, 49000.0]
        }
    ]
...
def assert_performance():
    ...
    def _assert_performance(operation_time: float, threshold: float, operation_name: str):
        ...
        assert operation_time <= threshold, (
            f"{operation_name} took {operation_time:.3f}s, "
...
def capture_logs():
    ...
    def _capture_logs(logger_name: str = None):
