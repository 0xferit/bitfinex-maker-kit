...
from bitfinex_maker_kit.utilities.constants import DEFAULT_SYMBOL
...
def paper_trading_available():
    ...
    return (
        os.environ.get("BFX_API_PAPER_KEY") is not None and 
        os.environ.get("BFX_API_PAPER_SECRET") is not None
    )
...
class TestCommandsWithMocks:
    ...
    def test_wallet_command_success(self, mock_container):
        """Test wallet command executes successfully."""
        container, trading_service, client = mock_container
        
        with patch("bitfinex_maker_kit.commands.wallet.get_container", return_value=container):
            wallet_command()
    
    def test_list_command_success(self, mock_container):
        """Test list command executes successfully."""
        container, trading_service, client = mock_container
        
        with patch("bitfinex_maker_kit.commands.list.get_container", return_value=container):
            list_command()
    
    def test_list_command_with_symbol(self, mock_container):
        """Test list command with specific symbol."""
        container, trading_service, client = mock_container
        
        with patch("bitfinex_maker_kit.commands.list.get_container", return_value=container):
            list_command(symbol="tETHUSD")
            trading_service.get_orders.assert_called()
    ...
    def test_market_make_command_basic(self, mock_container):
        ...
        with patch("bitfinex_maker_kit.commands.market_make.get_container", return_value=container):
            market_make_command(
                symbol="tBTCUSD",
                levels=3,
                spread=1.0,
    ...
    def test_update_command_price_change(self, mock_container):
        ...
        with patch("bitfinex_maker_kit.commands.update.get_container", return_value=container):
            update_command(
                order_id=12345,
                new_price=51000.0,
                dry_run=True
    ...
    def test_fill_spread_command_basic(self, mock_container):
        ...
        with patch("bitfinex_maker_kit.commands.fill_spread.get_container", return_value=container):
            fill_spread_command(
                symbol="tBTCUSD",
                levels=5,
                dry_run=True
    ...
    def test_monitor_command_basic(self, mock_container):
        ...
        with patch("bitfinex_maker_kit.commands.monitor.get_container", return_value=container):
            # Monitor command should execute without hanging
            monitor_command(symbol="tBTCUSD", once=True)
    ...
    def test_put_then_list_workflow(self, mock_setup):
        ...
        with patch("bitfinex_maker_kit.commands.put.get_container", return_value=container):
            with patch("bitfinex_maker_kit.commands.list.get_container", return_value=container):
                ...
                # List orders
                list_command(symbol="tBTCUSD")
                
                # Should have called both operations
                assert client.submit_order.called or trading_service.get_orders.called
...
class TestCommandProperties:
    ...
    def test_all_commands_handle_missing_credentials(self):
        ...
        for cmd_name, cmd_func, args in commands_to_test:
