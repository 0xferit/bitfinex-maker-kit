...
from bitfinex_maker_kit.services.container import create_service_container
from bitfinex_maker_kit.services.monitored_trading_service import create_monitored_trading_service

from ..mocks.service_mocks import create_mock_cache_service, create_mock_performance_monitor
...
class TestTradingServiceIntegration:
    ...
    async def monitored_trading_service(self, service_container):
        ...
        service = create_monitored_trading_service(
            service_container, performance_monitor=performance_monitor
        )
    ...
    async def test_place_order_integration(
        ...
        result = await trading_service.place_order(
            symbol=sample_symbol, amount=sample_amount, price=sample_price, side="buy"
        )
...
class TestConcurrencyIntegration:
    ...
    async def test_concurrent_order_placement(self, trading_service):
        ...
        tasks = [trading_service.place_order(**order_params) for order_params in orders]
    ...
    async def test_concurrent_cache_operations(self, cache_service):
        ...
        set_tasks = [cache_service.set("test", f"key_{i}", f"value_{i}") for i in range(10)]

        await asyncio.gather(*set_tasks)

        # Concurrent get operations
        get_tasks = [cache_service.get("test", f"key_{i}") for i in range(10)]
