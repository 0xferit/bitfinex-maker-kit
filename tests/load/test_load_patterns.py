...
class LoadPatternConfig:
    ...
    ramp_up_time_seconds: Optional[float] = None
...
class LoadPatternTester:
    ...
    async def run_load_pattern_test(self, config: LoadPatternConfig,
        ...
        while time.time() < end_time:
            ...
            if current_second != last_second:
                ...
                await self._execute_operations_batch(
                    operation_factory, operations_this_second,
        ...
        print(f"Completed load pattern test: {test_name} - {result['operations_per_second']:.1f} ops/sec")
        
        return result
    
    async def _execute_operations_batch(self, operation_factory: Callable,
...
class TestTradingLoadPatterns:
    ...
    async def test_constant_trading_load(self, load_tester):
        ...
        assert result['success_rate'] >= 0.8, f"Constant load success rate too low: {result['success_rate']:.1%}"
        assert result['operations_per_second'] >= 3.0, f"Constant load throughput too low: {result['operations_per_second']:.1f} ops/sec"
    ...
    async def test_ramp_up_trading_load(self, load_tester):
        ...
        assert result['success_rate'] >= 0.7, f"Ramp-up load success rate too low: {result['success_rate']:.1%}"
        assert result['operations_per_second'] >= 5.0, f"Ramp-up load throughput too low: {result['operations_per_second']:.1f} ops/sec"
    ...
    async def test_spike_trading_load(self, load_tester):
        ...
        assert result['success_rate'] >= 0.6, f"Spike load success rate too low: {result['success_rate']:.1%}"
        assert result['operations_per_second'] >= 4.0, f"Spike load throughput too low: {result['operations_per_second']:.1f} ops/sec"
    ...
    async def test_burst_trading_load(self, load_tester):
        ...
        assert result['success_rate'] >= 0.7, f"Burst load success rate too low: {result['success_rate']:.1%}"
        assert result['operations_per_second'] >= 5.0, f"Burst load throughput too low: {result['operations_per_second']:.1f} ops/sec"
        
        # Verify burst pattern has high variation
        load_samples = result['load_samples']
        target_operations = [s['target_operations'] for s in load_samples]
        if len(target_operations) > 1:
            operation_variance = sum((x - sum(target_operations)/len(target_operations))**2 for x in target_operations) / len(target_operations)
...
class TestCacheLoadPatterns:
    ...
    async def test_wave_cache_load(self, load_tester):
        ...
        try:
            ...
            assert result['success_rate'] >= 0.95, f"Wave cache load success rate too low: {result['success_rate']:.1%}"
            assert result['operations_per_second'] >= 10.0, f"Wave cache load throughput too low: {result['operations_per_second']:.1f} ops/sec"
    ...
    async def test_random_cache_load(self, load_tester):
        ...
        try:
            ...
            assert result['success_rate'] >= 0.90, f"Random cache load success rate too low: {result['success_rate']:.1%}"
            assert result['operations_per_second'] >= 15.0, f"Random cache load throughput too low: {result['operations_per_second']:.1f} ops/sec"
            ...
            if len(target_operations) > 2:
                ...
                assert variety_ratio >= 0.3, f"Random pattern not sufficiently varied: {variety_ratio:.2f}"
...
class TestMixedLoadPatterns:
    ...
    async def test_mixed_system_ramp_up(self, load_tester):
        ...
        try:
                ...
                def operation():
                    ...
                    if operation_counter % 2 == 0:
                        ...
                        if random.random() < 0.7:  # 70% reads
                            ...
                            return cache_service.set('mixed_system', key, f'value_{operation_counter}')
            ...
            assert result['success_rate'] >= 0.75, f"Mixed system load success rate too low: {result['success_rate']:.1%}"
            assert result['operations_per_second'] >= 8.0, f"Mixed system load throughput too low: {result['operations_per_second']:.1f} ops/sec"
            
            # System should handle mixed load effectively
            assert result['avg_operation_time'] < 1.0, f"Mixed system operation time too high: {result['avg_operation_time']:.3f}s"
    ...
    async def test_complex_burst_pattern(self, load_tester):
        ...
        try:
                    ...
                    if operation_type == 'place_order':
                        ...
                        async def batch_cache_ops():
                            ...
                            for i in range(3):
                                ...
                                results.append(await cache_service.set('complex_test', key, f'batch_value_{i}'))
                            return all(results)
            ...
            assert result['success_rate'] >= 0.6, f"Complex burst pattern success rate too low: {result['success_rate']:.1%}"
            assert result['operations_per_second'] >= 8.0, f"Complex burst pattern throughput too low: {result['operations_per_second']:.1f} ops/sec"
