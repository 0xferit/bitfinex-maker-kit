...
class StressTestRunner:
    ...
    async def run_stress_test(self, test_name: str, stress_function, 
                            duration_seconds: float = 30.0,
                            metadata: Optional[Dict[str, Any]] = None) -> StressTestResult:
        """
        ...
        print(f"Completed stress test: {test_name} - {ops_per_second:.1f} ops/sec, {success_rate:.1%} success")
...
class TestTradingStressScenarios:
    ...
    async def test_high_frequency_order_placement(self, stress_runner):
        ...
        assert result.operations_per_second >= 10, f"HFT throughput too low: {result.operations_per_second:.1f} ops/sec"
    ...
    async def test_massive_concurrent_orders(self, stress_runner):
        ...
        assert result.success_rate >= 0.5, f"Concurrent order success rate too low: {result.success_rate:.1%}"
        assert result.operations_per_second >= 1, f"Concurrent throughput too low: {result.operations_per_second:.1f} ops/sec"
    ...
    async def test_order_cancel_storm(self, stress_runner):
        ...
        assert result.operations_per_second >= 15, f"Cancel throughput too low: {result.operations_per_second:.1f} ops/sec"
    ...
    async def test_mixed_operation_chaos(self, stress_runner):
        ...
        assert result.operations_per_second >= 5, f"Chaos throughput too low: {result.operations_per_second:.1f} ops/sec"
...
class TestCacheStressScenarios:
    ...
    async def test_cache_write_storm(self, stress_runner):
        ...
        try:
            ...
            assert result.operations_per_second >= 50, f"Cache write throughput too low: {result.operations_per_second:.1f} ops/sec"
            assert result.success_rate >= 0.95, f"Cache write success rate too low: {result.success_rate:.1%}"
    ...
    async def test_cache_read_storm(self, stress_runner):
        ...
        try:
            ...
            assert result.operations_per_second >= 200, f"Cache read throughput too low: {result.operations_per_second:.1f} ops/sec"
            assert result.success_rate >= 0.98, f"Cache read success rate too low: {result.success_rate:.1%}"
    ...
    async def test_cache_mixed_load_storm(self, stress_runner):
        ...
        try:
            ...
            assert result.operations_per_second >= 100, f"Mixed cache throughput too low: {result.operations_per_second:.1f} ops/sec"
            assert result.success_rate >= 0.90, f"Mixed cache success rate too low: {result.success_rate:.1%}"
...
class TestSystemIntegrationStress:
    ...
    async def test_full_system_load(self, stress_runner):
        ...
        try:
            ...
            async def realistic_trading_session():
                ...
                try:
                    ...
                    if cached_price is None:
                        ...
                        await cache_service.set('market_data', f'{symbol}_price', current_price, ttl=10.0)
                    else:
            ...
            assert result.operations_per_second >= 5, f"System throughput too low: {result.operations_per_second:.1f} ops/sec"
            assert result.success_rate >= 0.8, f"System success rate too low: {result.success_rate:.1%}"
            assert result.peak_memory_mb < 200, f"System memory usage too high: {result.peak_memory_mb:.1f}MB"
    ...
    async def test_resource_exhaustion_resilience(self, stress_runner):
        ...
        try:
            ...
            assert result.operations_per_second >= 1, f"Resource-constrained throughput too low: {result.operations_per_second:.1f} ops/sec"
            assert result.success_rate >= 0.5, f"Resource-constrained success rate too low: {result.success_rate:.1%}"
            
            # System should survive resource pressure
            assert len(result.error_details) < result.total_operations, "System completely failed under resource pressure"
