...
class TestHighVolumeTrading:
    ...
    async def test_high_frequency_order_placement(self, trading_service):
        ...
        assert avg_response_time < 100.0, f"Average response time {avg_response_time:.1f}ms too high"
    ...
    async def test_bulk_order_management(self, trading_service):
        ...
        assert overall_throughput >= 50, f"Bulk throughput {overall_throughput:.1f} orders/sec too low"
    ...
    async def test_concurrent_user_simulation(self, trading_service):
        ...
        async def simulate_user_activity(user_id: int):
            ...
            for op_num in range(operations_per_user):
                ...
                user_stats['response_times'].append(
        ...
        assert operations_per_second >= 15, f"Concurrent throughput {operations_per_second:.1f} ops/sec too low"
        assert avg_response_time < 500.0, f"Average response time {avg_response_time:.1f}ms too high"
...
class TestCacheLoadTests:
    ...
    async def test_concurrent_cache_access(self, cache_service):
        ...
        assert ops_per_second >= 500, f"Concurrent cache throughput {ops_per_second:.1f} ops/sec too low"
...
class TestSystemStressTests:
        ...
        for i in range(100):
            try:
                if i % 5 == 0:  # 20% error rate
                    ...
                    trading_service._trading_service.get_client().submit_order.side_effect = Exception("Simulated error")
                else:
    ...
    async def test_resource_exhaustion_handling(self):
        ...
        try:
            ...
            assert success_rate >= 0.95, f"Resource exhaustion success rate {success_rate:.2%} too low"
            assert ops_per_second >= 50, f"Resource exhaustion throughput {ops_per_second:.1f} ops/sec too low"
