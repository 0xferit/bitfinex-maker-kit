...
def trading_amounts(draw):
    ...
    amount_strategies = st.one_of(
        ...
        st.decimals(min_value=Decimal("10"), max_value=Decimal("1000"), places=2),    # Large retail/institutional
    )
...
def market_making_setups(draw):
    ...
    center_price = draw(st.decimals(min_value=Decimal("100"), max_value=Decimal("100000"), places=2))
...
class TradingOperationsStateMachine(RuleBasedStateMachine):
    ...
    @rule(target=active_orders, symbol=symbols, 
          price_str=market_prices(), amount_str=trading_amounts())
    ...
    def order_data_consistency(self):
        ...
        for order_id, order_data in self.placed_orders.items():
