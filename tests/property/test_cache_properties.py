...
class TestCacheConsistencyProperties:
    ...
    @given(cache_namespaces(), st.lists(st.tuples(cache_keys(), cache_values()), min_size=1, max_size=20))
        ...
        try:
            ...
            while time.time() - start_time < ttl + 0.5:  # Allow 500ms tolerance
                ...
                if elapsed < ttl - 0.1:  # Before expiration (with tolerance)
                    assert retrieved == value, f"Value expired too early at {elapsed:.3f}s (TTL: {ttl}s)"
...
class TestCachePerformanceProperties:
    ...
    async def test_bulk_operations_performance(self, key_value_pairs):
        ...
        try:
            ...
            assert set_ops_per_second >= 100, f"Set performance too low: {set_ops_per_second:.1f} ops/sec"
            assert get_ops_per_second >= 500, f"Get performance too low: {get_ops_per_second:.1f} ops/sec"
    ...
    async def test_concurrent_access_performance(self, key, value, num_concurrent):
        ...
        try:
            ...
            assert ops_per_second >= 50, f"Concurrent performance too low: {ops_per_second:.1f} ops/sec"
...
class TestCacheEdgeCases:
    ...
    @given(cache_namespaces(), cache_keys(), 
