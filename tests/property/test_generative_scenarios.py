...
CI_SETTINGS = settings(max_examples=8, deadline=15000)
...
def market_conditions(draw):
    ...
    return {
        ...
        "base_price": float(
            draw(st.decimals(min_value=Decimal("100"), max_value=Decimal("100000"), places=2))
        ),
        "spread_bps": draw(st.integers(min_value=1, max_value=500)),
        "volume_multiplier": float(
            draw(st.decimals(min_value=Decimal("0.1"), max_value=Decimal("10.0"), places=2))
        ),
    }
...
def trading_strategies(draw):
    ...
    if strategy_type == "market_making":
        return {
            ...
            "spread_pct": float(
                draw(st.decimals(min_value=Decimal("0.01"), max_value=Decimal("2.0"), places=3))
            ),
            "order_size": float(
                draw(st.decimals(min_value=Decimal("0.01"), max_value=Decimal("10.0"), places=6))
            ),
            "max_inventory": float(
                draw(st.decimals(min_value=Decimal("1.0"), max_value=Decimal("100.0"), places=2))
            ),
        }
        ...
        return {
            ...
            "max_position": float(
                draw(st.decimals(min_value=Decimal("1.0"), max_value=Decimal("50.0"), places=2))
            ),
        ...
        return {
            ...
            "threshold_pct": float(
                draw(st.decimals(min_value=Decimal("0.1"), max_value=Decimal("5.0"), places=2))
            ),
            "position_size": float(
                draw(st.decimals(min_value=Decimal("0.1"), max_value=Decimal("20.0"), places=2))
            ),
        }
...
def user_behavior_patterns(draw):
    ...
    return {
        ...
        "error_tolerance": float(
            draw(st.decimals(min_value=Decimal("0.01"), max_value=Decimal("0.20"), places=3))
        ),
        "session_duration": draw(st.integers(min_value=60, max_value=3600)),
    }
...
class TestComplexTradingScenarios:
    ...
    async def _test_market_making_strategy(self, trading_service, market_cond, strategy):
        ...
        for level in range(1, levels + 1):
            volatility_multiplier = {"low": 1.0, "medium": 1.5, "high": 2.0, "extreme": 3.0}.get(
                market_cond["volatility"], 1.0
            )
    ...
    async def _test_arbitrage_strategy(self, trading_service, market_cond, strategy):
        ...
        min_spread = strategy["min_spread_bps"] / 10000
        max_position = strategy["max_position"]

        base_price = market_cond["base_price"]
        spread = base_price * min_spread * 2
        ...
        try:
            ...
            expected_profit = (
                float(sell_order["price"]) - float(buy_order["price"])
            ) * position_size
    ...
    async def test_multi_user_scenarios(self, user_patterns):
        ...
        async def simulate_user(user_pattern):
            ...
            order_counts = {"low": (1, 5), "medium": (5, 15), "high": (15, 30), "burst": (30, 50)}
...
class TestSystemStressScenarios:
    ...
    async def test_trading_stress(self, max_orders):
...
class TradingSystemScenarioMachine(RuleBasedStateMachine):
    ...
    def system_consistency(self):
        ...
        assert (
            self.system_state["total_orders_cancelled"] <= self.system_state["total_orders_placed"]
        )
...
class TestGenerativeEdgeCases:
    ...
    async def test_random_operation_sequences(self, data):
        ...
        for _ in range(num_operations):
            operation = data.draw(st.sampled_from(["place_order", "cancel_order", "get_orders"]))
    ...
    async def test_concurrent_operation_chaos(self, data):
        ...
        async def chaotic_worker(worker_id):
            ...
            for _i in range(operations):
                try:
                    op_type = data.draw(st.sampled_from(["trading", "query"]))
        ...
        assert len(completed_workers) >= 0
