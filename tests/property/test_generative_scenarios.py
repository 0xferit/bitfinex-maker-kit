...
def system_configurations(draw):
    ...
    return {
        ...
        "cache_ttl": float(
            draw(st.decimals(min_value=Decimal("1.0"), max_value=Decimal("3600.0"), places=1))
        ),
        ...
        "order_timeout": float(
            draw(st.decimals(min_value=Decimal("1.0"), max_value=Decimal("300.0"), places=1))
        ),
            ...
            async def chaotic_worker(worker_id):
                ...
                for i in range(operations):
                    try:
                        ...
                        op_type = data.draw(st.sampled_from(["trading", "cache", "query"]))
