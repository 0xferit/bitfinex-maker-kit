...
class PerformanceRegressionChecker:
    ...
    def analyze_regressions(
        ...
        regressions.sort(
            key=lambda x: (severity_order[x.severity], x.regression_percent), reverse=True
        )

        return regressions

    def generate_report(
        self, regressions: list[RegressionResult], baseline_path: str, current_path: str
    ) -> dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "has_blocking_regressions": any(
                    r.severity in ["critical", "high"] for r in regressions
                ),
            },
            "regressions": [],
        }
...
def main():
    ...
    parser.add_argument(
        "--threshold", type=float, default=0.15, help="Regression threshold (default: 0.15 = 15%)"
    )
    ...
    if args.verbose:
        ...
        if regressions:
            ...
            for regression in regressions:
                print(
                    f"  {regression.function_name}: {regression.regression_percent:.1%} slower "
